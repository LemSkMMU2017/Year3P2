...
def convergence_rates(m, solver_function, num_periods=8):
    ...
    from math import pi
    omega = 0.35;
    U = 0.3  # просто заданные значения
    P = 2 * pi / omega  # период
...
def main(solver_function=solver):
    ...
    T = P * num_periods
    u1, t1 = solver_function(U, omega, tau, T)
    u2, t2 = solver_function(U, omega - tau ** 2 * omega ** 3 / 24, tau, T)
    if num_periods <= 10:
...
def demo_bokeh():
    ...
    for n in num_steps_per_period:
        tau = P / n
        u_, t_ = solver(U=1, omega = omega - tau ** 2 * omega ** 3 / 24, tau=tau, T=T)
        u.append(u_)
        t.append(t_)
        legends.append(u'Количество шагов на период: %d' % n)
    bokeh_plot(u, t, legends, U=1, omega=omega, t_range=[0, 4 * P],

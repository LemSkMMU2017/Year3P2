...
t, omega, tau, p = sym.symbols('V t U omega tau i p')
...
def tailor(E):
    return E(math.pi) + sym.diff(((2 / p) ** 2) * (sym.sin(p / 2)) ** 2, p).subs(p, math.pi) * \
            (p + math.pi) + \
...
if __name__ == "__main__":
    ...
    with open("dotsE.dat", "w") as f:
        for p in s:
            f.write("" + str(p) + " " + str(countE(p)) + "\n")

    with open("tailor.dat", "w") as f:
        f.write(str(tailor(E)) + "\n")

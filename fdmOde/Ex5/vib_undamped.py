...
def main(solver_function=solver):
    ...
    T = P*num_periods
    u1, t1 = solver_function(U, omega, tau, T, 0)
    u2, t2 = solver_function(U, omega, tau, T, 1)
    if num_periods <= 10:
        visualize(u1, t1, u2, t2, U, omega)
    else:
        visualize_front(u1, t1, u2, t2, U, omega, savefig)
...
def demo_bokeh():
    ...
    for n in num_steps_per_period:
        tau = P/n
        u_, t_ = solver(U=1, omega=omega, tau=tau, T=T, flag=0)
        u.append(u_)
        t.append(t_)
    ...
    for n in num_steps_per_period:
        tau = P / n
        u_, t_ = solver(U=1, omega=omega, tau=tau, T=T, flag=1)
        u.append(u_)
        t.append(t_)
        legends.append(u'Количество шагов на период: %d' % n)
    bokeh_plot(u, t, legends, U=1, omega=omega, t_range=[0, 4 * P],
